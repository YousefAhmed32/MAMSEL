    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{t('brands.title')}</h1>
            <p className="text-gray-600 dark:text-luxury-gold/70">{t('brands.subtitle')}</p>
          </div>
        </div>
        ...
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          ...
          <DialogContent className="max-w-2xl bg-white dark:bg-luxury-navy/95 backdrop-blur-sm border-luxury-gold/30">
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold text-gray-900 dark:text-luxury-gold">{t('brands.addNewBrand')}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div>
                <label className="text-gray-900 dark:text-white font-semibold mb-2 block">{t('brands.brandNameAr')}</label>
                <Input
                  value={formData.name}
                  onChange={(e) => setFormData({...formData, name: e.target.value})}
                  placeholder={t('brands.enterBrandNameAr')}
                  className="bg-white dark:bg-luxury-navy/50 border-luxury-gold/30 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-white/50"
                />
              </div>
              <div>
                <label className="text-gray-900 dark:text-white font-semibold mb-2 block">{t('brands.brandNameEn')}</label>
                <Input
              ...
              <div>
                <label className="text-gray-900 dark:text-white font-semibold mb-2 block">{t('brands.description')}</label>
                <Input
              ...
              <div>
                <label className="text-gray-900 dark:text-white font-semibold mb-2 block">{t('brands.logoLink')}</label>
                <Input
                  value={formData.logo}
      ...
      <div className="relative">
        ...
        <Input
          ...
          className="bg-white/90 dark:bg-luxury-navy/50 border-luxury-gold/30 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-white/50 pr-12"
        />
      </div>
      ...
      {filteredBrands.length === 0 ? (
        <div className="text-center py-12">
          <Building2 className="w-16 h-16 mx-auto text-luxury-gold/50 mb-4" />
          <p className="text-gray-600 dark:text-gray-400 text-lg">{t('brands.noBrands')}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBrands.map((brand) => (
          <Card key={brand._id} className=" bg-white/90  dark:bg-luxury-navy/20 backdrop-blur-sm border-luxury-gold/20 hover:border-luxury-gold/40 transition-all duration-300">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-gray-900 dark:text-white text-lg">{brand.name}</CardTitle>
                    <p className="text-gray-600 dark:text-luxury-gold/70 text-sm">{brand.nameEn}</p>
                  </div>
                </div>
                <Badge
            ...
            <CardContent className="pt-0">
              {brand.description && (
                <p className="text-gray-600 dark:text-white/70 text-sm mb-4">{brand.description}</p>
              )}
              <div className="flex items-center justify-between">
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl bg-white dark:bg-luxury-navy/95 backdrop-blur-sm border-luxury-gold/30">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-gray-900 dark:text-luxury-gold">{t('brands.editBrand')}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <label className="text-gray-900 dark:text-white font-semibold mb-2 block">{t('brands.brandNameAr')}</label>
